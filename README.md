...
**Phase 4a (admin dashboard + RBAC) is also complete:** `/admin` with
role-based access control (Super Admin, Product Manager, Order Manager,
Customer Support), analytics, orders, products, categories, collections,
customers, inventory, coupons, reviews, admins/roles, and audit logs.
Log in as `admin@sanaera.com` (password `ChangeMe!123` from the seed script —
**change this immediately**) to see it. Returns/banners/homepage-editor/
reports are Phase 4b, not built yet — see `ARCHITECTURE.md`.

**Phase 5 (product/inventory management + Cloudinary) is also complete:**
real product create/edit forms with image upload and variants at
`/admin/products/new` and `/admin/products/[id]/edit`, a daily low-stock
email via Vercel Cron, and avatar uploads on `/account/profile`. **The
public storefront still reads from the `lib/products.ts` mock array** —
migrating it to the database is flagged as the next step, not done here.
See `ARCHITECTURE.md` for why that's a deliberate cut, not an oversight.
